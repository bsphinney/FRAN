...
(~2.7M precursors and growing). GPMDB-inspired in *function* — browse by
peptide, protein/gene, organism, run — but built for 2026: live counts you can
watch grow as ingest proceeds, an RT × ion-mobility (1/K₀) showcase GPMDB never
had, light/dark, server-side aggregation, and a hard public-layer security
boundary.
...
2. **Public-layer allowlist.** Every query declares which tables it reads; the
   set is validated against `PUBLIC_TABLES`. The internal tables
   (`delimp_searches_internal`, `delimp_raw_files_internal`) are deliberately
   absent — they cannot be reached through this app.
