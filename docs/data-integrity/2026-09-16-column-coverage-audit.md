...
- `delimp_precursors.intensity_log2` had **no writer at all**, yet two user-facing surfaces read
  it: the peptide page's `Avg log₂ int` column (an em-dash on every peptide in the corpus) and the
  Ion Mobility scatter's intensity dimension. A missing value renders as a dash rather than
  throwing, so the omission survived into production on both.
